.class public abstract Lfud;
.super Ljava/lang/Object;
.source "SourceFile"


# direct methods
.method public static f(Ljava/lang/Throwable;)Ln3a;
    .locals 2

    const-string v0, "throwable is null"

    invoke-static {p0, v0}, Ljava/util/Objects;->requireNonNull(Ljava/lang/Object;Ljava/lang/String;)Ljava/lang/Object;

    new-instance v0, Lma6;

    invoke-direct {v0, p0}, Lma6;-><init>(Ljava/lang/Object;)V

    new-instance p0, Ln3a;

    const/4 v1, 0x3

    invoke-direct {p0, v1, v0}, Ln3a;-><init>(ILjava/lang/Object;)V

    return-object p0
.end method

.method public static g(Ljava/lang/Object;)Ln3a;
    .locals 2

    const-string v0, "item is null"

    invoke-static {p0, v0}, Ljava/util/Objects;->requireNonNull(Ljava/lang/Object;Ljava/lang/String;)Ljava/lang/Object;

    new-instance v0, Ln3a;

    const/4 v1, 0x5

    invoke-direct {v0, v1, p0}, Ln3a;-><init>(ILjava/lang/Object;)V

    return-object v0
.end method


# virtual methods
.method public final e()Ljava/lang/Object;
    .locals 2

    new-instance v0, Lfq0;

    const/4 v1, 0x1

    invoke-direct {v0, v1}, Ljava/util/concurrent/CountDownLatch;-><init>(I)V

    invoke-virtual {p0, v0}, Lfud;->k(Lzud;)V

    invoke-virtual {v0}, Lfq0;->d()Ljava/lang/Object;

    move-result-object p0

    return-object p0
.end method

.method public final h(Lu96;)Lpud;
    .locals 2

    const-string v0, "mapper is null"

    invoke-static {p1, v0}, Ljava/util/Objects;->requireNonNull(Ljava/lang/Object;Ljava/lang/String;)Ljava/lang/Object;

    new-instance v0, Lpud;

    const/4 v1, 0x1

    invoke-direct {v0, p0, p1, v1}, Lpud;-><init>(Lfud;Lu96;I)V

    return-object v0
.end method

.method public final i(Lvxc;)Lyud;
    .locals 2

    const-string v0, "scheduler is null"

    invoke-static {p1, v0}, Ljava/util/Objects;->requireNonNull(Ljava/lang/Object;Ljava/lang/String;)Ljava/lang/Object;

    new-instance v0, Lyud;

    const/4 v1, 0x0

    invoke-direct {v0, p0, p1, v1}, Lyud;-><init>(Lfud;Lvxc;I)V

    return-object v0
.end method

.method public final j(Lwpe;)Lqu5;
    .locals 2

    instance-of v0, p0, Lpa6;

    if-eqz v0, :cond_0

    check-cast p0, Lpa6;

    invoke-interface {p0}, Lpa6;->b()Lkt5;

    move-result-object p0

    goto :goto_0

    :cond_0
    new-instance v0, Lut5;

    const/4 v1, 0x3

    invoke-direct {v0, v1, p0}, Lut5;-><init>(ILjava/lang/Object;)V

    move-object p0, v0

    :goto_0
    new-instance v0, Llu5;

    invoke-direct {v0, p0, p1}, Llu5;-><init>(Lkt5;Lwpe;)V

    new-instance p0, Lqu5;

    const/4 p1, 0x0

    invoke-direct {p0, v0, p1}, Lqu5;-><init>(Lo0;I)V

    return-object p0
.end method

.method public final k(Lzud;)V
    .locals 1

    const-string v0, "observer is null"

    invoke-static {p1, v0}, Ljava/util/Objects;->requireNonNull(Ljava/lang/Object;Ljava/lang/String;)Ljava/lang/Object;

    :try_start_0
    invoke-virtual {p0, p1}, Lfud;->l(Lzud;)V
    :try_end_0
    .catch Ljava/lang/NullPointerException; {:try_start_0 .. :try_end_0} :catch_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    return-void

    :catchall_0
    move-exception p0

    invoke-static {p0}, Lve2;->b0(Ljava/lang/Throwable;)V

    new-instance p1, Ljava/lang/NullPointerException;

    const-string v0, "subscribeActual failed"

    invoke-direct {p1, v0}, Ljava/lang/NullPointerException;-><init>(Ljava/lang/String;)V

    invoke-virtual {p1, p0}, Ljava/lang/Throwable;->initCause(Ljava/lang/Throwable;)Ljava/lang/Throwable;

    throw p1

    :catch_0
    move-exception p0

    throw p0
.end method

.method public abstract l(Lzud;)V
.end method

.method public final m(Lvxc;)Lyud;
    .locals 2

    const-string v0, "scheduler is null"

    invoke-static {p1, v0}, Ljava/util/Objects;->requireNonNull(Ljava/lang/Object;Ljava/lang/String;)Ljava/lang/Object;

    new-instance v0, Lyud;

    const/4 v1, 0x1

    invoke-direct {v0, p0, p1, v1}, Lyud;-><init>(Lfud;Lvxc;I)V

    return-object v0
.end method

.method public final n()Lt0a;
    .locals 2

    instance-of v0, p0, Lqa6;

    if-eqz v0, :cond_0

    check-cast p0, Lqa6;

    invoke-interface {p0}, Lqa6;->d()Lt0a;

    move-result-object p0

    return-object p0

    :cond_0
    new-instance v0, Lnc3;

    const/16 v1, 0x8

    invoke-direct {v0, v1, p0}, Lnc3;-><init>(ILjava/lang/Object;)V

    return-object v0
.end method
