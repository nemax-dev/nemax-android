.class public final synthetic Lp02;
.super Ljava/lang/Object;
.source "SourceFile"

# interfaces
.implements Lzm3;


# instance fields
.field public final synthetic a:I

.field public final synthetic b:Ljava/lang/Object;


# direct methods
.method public synthetic constructor <init>(ILjava/lang/Object;)V
    .locals 0

    iput p1, p0, Lp02;->a:I

    iput-object p2, p0, Lp02;->b:Ljava/lang/Object;

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method


# virtual methods
.method public final accept(Ljava/lang/Object;)V
    .locals 6

    iget v0, p0, Lp02;->a:I

    const/4 v1, 0x2

    const/4 v2, 0x0

    const/4 v3, 0x1

    const/4 v4, 0x0

    iget-object p0, p0, Lp02;->b:Ljava/lang/Object;

    packed-switch v0, :pswitch_data_0

    check-cast p0, Lt2g;

    check-cast p1, Ly5g;

    instance-of v0, p1, Lw5g;

    if-eqz v0, :cond_3

    iget-object p1, p0, Lt2g;->r0:Ljava/lang/String;

    sget-object v0, Lkug;->g:Leka;

    if-nez v0, :cond_0

    goto :goto_0

    :cond_0
    sget-object v1, Le08;->o:Le08;

    invoke-virtual {v0, v1}, Leka;->a(Le08;)Z

    move-result v5

    if-eqz v5, :cond_1

    const-string v5, "VideoMessage Recording. VideoRecordEvent.Start recording start"

    invoke-virtual {v0, v1, p1, v5, v4}, Leka;->b(Le08;Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)V

    :cond_1
    :goto_0
    iget-object p1, p0, Lt2g;->C0:Lapc;

    if-eqz p1, :cond_2

    iget-object p1, p0, Lt2g;->s0:Lkotlinx/coroutines/internal/ContextScope;

    sget-object v0, Li14;->b:Li14;

    new-instance v1, Lp2g;

    invoke-direct {v1, p0, v4}, Lp2g;-><init>(Lt2g;Lkotlin/coroutines/Continuation;)V

    invoke-static {p1, v4, v0, v1, v3}, Lvzg;->s(Lf14;Lx04;Li14;Lad6;I)Lwae;

    move-result-object p1

    iget-object v0, p0, Lt2g;->U0:Lqod;

    sget-object v1, Lt2g;->X0:[Lqj7;

    aget-object v1, v1, v2

    invoke-virtual {v0, p0, v1, p1}, Lqod;->h0(Ljava/lang/Object;Lqj7;Ljava/lang/Object;)V

    :cond_2
    iget-object p0, p0, Lt2g;->G0:Ltde;

    sget-object p1, Lgzf;->a:Lgzf;

    invoke-virtual {p0, v4, p1}, Ltde;->m(Ljava/lang/Object;Ljava/lang/Object;)Z

    goto :goto_1

    :cond_3
    instance-of v0, p1, Lt5g;

    if-eqz v0, :cond_4

    iget-object v0, p0, Lt2g;->s0:Lkotlinx/coroutines/internal/ContextScope;

    invoke-virtual {p0}, Lt2g;->q()Luxe;

    move-result-object v2

    check-cast v2, Lqga;

    invoke-virtual {v2}, Lqga;->b()Lz04;

    move-result-object v2

    new-instance v3, Ls2g;

    invoke-direct {v3, p1, p0, v4}, Ls2g;-><init>(Ly5g;Lt2g;Lkotlin/coroutines/Continuation;)V

    invoke-static {v0, v2, v4, v3, v1}, Lvzg;->s(Lf14;Lx04;Li14;Lad6;I)Lwae;

    :cond_4
    :goto_1
    return-void

    :pswitch_0
    check-cast p0, Lvr4;

    check-cast p1, Lfb0;

    iget-object v0, p1, Lfb0;->b:Landroid/view/Surface;

    invoke-virtual {v0}, Ljava/lang/Object;->hashCode()I

    iget-object p1, p1, Lfb0;->b:Landroid/view/Surface;

    iget-object v0, p0, Lvr4;->g:Ljava/lang/Object;

    check-cast v0, Landroid/view/Surface;

    if-ne p1, v0, :cond_5

    iput-object v4, p0, Lvr4;->g:Ljava/lang/Object;

    iget-object p1, p0, Lvr4;->n:Ljava/lang/Object;

    check-cast p1, Lps1;

    iget-object v0, p0, Lvr4;->f:Ljava/lang/Object;

    check-cast v0, Lo65;

    invoke-virtual {p1, v0}, Lps1;->b(Ljava/lang/Object;)Z

    invoke-virtual {p0}, Lvr4;->e()V

    goto :goto_2

    :cond_5
    invoke-virtual {p1}, Landroid/view/Surface;->release()V

    :goto_2
    return-void

    :pswitch_1
    check-cast p0, Lps1;

    check-cast p1, Lfb0;

    invoke-virtual {p0, p1}, Lps1;->b(Ljava/lang/Object;)Z

    return-void

    :pswitch_2
    check-cast p0, Lqz;

    check-cast p1, Lfb0;

    if-eqz p0, :cond_6

    invoke-virtual {p0}, Lqz;->k()V

    :cond_6
    return-void

    :pswitch_3
    check-cast p0, Ljava/util/Map;

    check-cast p1, Lgb0;

    invoke-interface {p0}, Ljava/util/Map;->entrySet()Ljava/util/Set;

    move-result-object p0

    invoke-interface {p0}, Ljava/util/Set;->iterator()Ljava/util/Iterator;

    move-result-object p0

    :goto_3
    invoke-interface {p0}, Ljava/util/Iterator;->hasNext()Z

    move-result v0

    if-eqz v0, :cond_8

    invoke-interface {p0}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/util/Map$Entry;

    iget v1, p1, Lgb0;->b:I

    invoke-interface {v0}, Ljava/util/Map$Entry;->getKey()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Lfa0;

    iget v2, v2, Lfa0;->f:I

    sub-int/2addr v1, v2

    invoke-interface {v0}, Ljava/util/Map$Entry;->getKey()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Lfa0;

    iget-boolean v2, v2, Lfa0;->g:Z

    if-eqz v2, :cond_7

    neg-int v1, v1

    :cond_7
    invoke-static {v1}, Lkef;->h(I)I

    move-result v1

    invoke-interface {v0}, Ljava/util/Map$Entry;->getValue()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lxre;

    invoke-virtual {v0}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    new-instance v2, Ljt0;

    const/4 v3, 0x4

    const/4 v4, -0x1

    invoke-direct {v2, v0, v1, v4, v3}, Ljt0;-><init>(Ljava/lang/Object;III)V

    invoke-static {v2}, Loe0;->R(Ljava/lang/Runnable;)V

    goto :goto_3

    :cond_8
    return-void

    :pswitch_4
    check-cast p0, Lnoc;

    check-cast p1, Landroid/net/Uri;

    iput-object p1, p0, Lnoc;->I:Landroid/net/Uri;

    return-void

    :pswitch_5
    check-cast p0, Lvr9;

    check-cast p1, Lrr9;

    iget-object p0, p0, Lvr9;->r0:Lcom/google/android/material/chip/ChipGroup;

    invoke-virtual {p0}, Lcom/google/android/material/chip/ChipGroup;->getCheckedChipId()I

    move-result p0

    sget v0, Lt8c;->layout_send_location__duration_20m:I

    if-ne p0, v0, :cond_9

    sget-object p0, Laz7;->c:Laz7;

    goto :goto_4

    :cond_9
    sget v0, Lt8c;->layout_send_location__duration_1h:I

    if-ne p0, v0, :cond_a

    sget-object p0, Laz7;->o:Laz7;

    goto :goto_4

    :cond_a
    sget v0, Lt8c;->layout_send_location__duration_3h:I

    if-ne p0, v0, :cond_b

    sget-object p0, Laz7;->X:Laz7;

    goto :goto_4

    :cond_b
    sget v0, Lt8c;->layout_send_location__duration_24h:I

    if-ne p0, v0, :cond_c

    sget-object p0, Laz7;->Y:Laz7;

    goto :goto_4

    :cond_c
    sget v0, Lt8c;->layout_send_location__duration_no_limit:I

    if-ne p0, v0, :cond_d

    sget-object p0, Laz7;->Z:Laz7;

    goto :goto_4

    :cond_d
    sget-object p0, Laz7;->X:Laz7;

    :goto_4
    invoke-virtual {p1}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iget-object p1, p1, Ly2;->a:Ljava/lang/Object;

    check-cast p1, Ljava/util/Set;

    invoke-interface {p1}, Ljava/util/Set;->iterator()Ljava/util/Iterator;

    move-result-object p1

    :goto_5
    invoke-interface {p1}, Ljava/util/Iterator;->hasNext()Z

    move-result v0

    if-eqz v0, :cond_e

    invoke-interface {p1}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lsq9;

    invoke-virtual {v0}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    new-instance v1, Lp02;

    const/4 v2, 0x3

    invoke-direct {v1, v2, p0}, Lp02;-><init>(ILjava/lang/Object;)V

    invoke-virtual {v0, v1}, Lsq9;->Y0(Lzm3;)V

    goto :goto_5

    :cond_e
    return-void

    :pswitch_6
    check-cast p0, Laz7;

    check-cast p1, Lzy7;

    iput-object p0, p1, Lzy7;->e:Laz7;

    return-void

    :pswitch_7
    check-cast p0, Ljava/lang/Boolean;

    check-cast p1, Lzy7;

    invoke-virtual {p0}, Ljava/lang/Boolean;->booleanValue()Z

    move-result p0

    iput-boolean p0, p1, Lzy7;->b:Z

    iput-boolean v3, p1, Lzy7;->a:Z

    return-void

    :pswitch_8
    check-cast p0, Lsq9;

    check-cast p1, Lzy7;

    iget-object p0, p0, Lsq9;->B0:Lbz7;

    iget p0, p0, Lbz7;->c:I

    iput p0, p1, Lzy7;->d:I

    iput-boolean v2, p1, Lzy7;->b:Z

    return-void

    :pswitch_9
    check-cast p0, Lru/ok/tamtam/android/widgets/quickcamera/CameraxCameraApiView;

    check-cast p1, Ly5g;

    sget v0, Lru/ok/tamtam/android/widgets/quickcamera/CameraxCameraApiView;->t0:I

    instance-of v0, p1, Lt5g;

    if-eqz v0, :cond_11

    check-cast p1, Lt5g;

    iget v0, p1, Lt5g;->c:I

    const-class v2, Lru/ok/tamtam/android/widgets/quickcamera/CameraxCameraApiView;

    if-eqz v0, :cond_f

    invoke-virtual {v2}, Ljava/lang/Class;->getName()Ljava/lang/String;

    move-result-object v0

    const-string v1, "onCameraError"

    invoke-static {v0, v1}, Lkug;->m(Ljava/lang/String;Ljava/lang/String;)V

    iget-object p0, p0, Lru/ok/tamtam/android/widgets/quickcamera/CameraxCameraApiView;->r0:Lqz1;

    if-eqz p0, :cond_11

    new-instance v0, Lru/ok/tamtam/android/widgets/quickcamera/CameraExceptionImpl;

    iget-object p1, p1, Lt5g;->d:Ljava/lang/Throwable;

    invoke-direct {v0, p1}, Ljava/lang/Exception;-><init>(Ljava/lang/Throwable;)V

    check-cast p0, Laha;

    invoke-virtual {p0, v0}, Laha;->E(Ldz1;)V

    goto :goto_6

    :cond_f
    invoke-virtual {v2}, Ljava/lang/Class;->getName()Ljava/lang/String;

    move-result-object v0

    const-string v2, "onVideoTaken"

    invoke-static {v0, v2}, Lkug;->m(Ljava/lang/String;Ljava/lang/String;)V

    iget-object p0, p0, Lru/ok/tamtam/android/widgets/quickcamera/CameraxCameraApiView;->r0:Lqz1;

    if-eqz p0, :cond_11

    iget-object p1, p1, Ly5g;->a:Ldn5;

    iget-object p1, p1, Ldn5;->b:Lo90;

    iget-object p1, p1, Lo90;->c:Ljava/io/File;

    check-cast p0, Laha;

    iget-object p0, p0, Laha;->b:Ljava/lang/Object;

    check-cast p0, Lh2c;

    invoke-static {p0}, Lh2c;->a(Lh2c;)Z

    move-result v0

    if-eqz v0, :cond_11

    iget-object p0, p0, Lh2c;->o:Lk2c;

    if-nez p0, :cond_10

    move-object p0, v4

    :cond_10
    iget-object v0, p0, Lk2c;->r0:Luxe;

    check-cast v0, Lqga;

    invoke-virtual {v0}, Lqga;->b()Lz04;

    move-result-object v0

    new-instance v2, Lj2c;

    invoke-direct {v2, p0, p1, v4}, Lj2c;-><init>(Lk2c;Ljava/io/File;Lkotlin/coroutines/Continuation;)V

    invoke-static {p0, v0, v2, v1}, Ly8g;->n(Ly8g;Lx04;Lad6;I)Lwae;

    :cond_11
    :goto_6
    return-void

    :pswitch_data_0
    .packed-switch 0x0
        :pswitch_9
        :pswitch_8
        :pswitch_7
        :pswitch_6
        :pswitch_5
        :pswitch_4
        :pswitch_3
        :pswitch_2
        :pswitch_1
        :pswitch_0
    .end packed-switch
.end method
